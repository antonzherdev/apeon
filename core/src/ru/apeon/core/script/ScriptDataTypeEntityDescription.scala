...
case class ScriptDataTypeEntityDescription(description : Description) extends ScriptDataType {
  ...
  override val declarations = Seq(applyId, applyEql, insert, findEql, findId) ++ obj.map(_.declarations).getOrElse(Seq())
  ...
  class FindId extends Declaration {
    def value(env: Environment, parameters: Option[Seq[ParVal]], dataSource: Option[Expression]) : Any =
      parameters.get.head.value match {
        case id : Int => env.em.get(new SqlEntityId(env.dataSource(dataSource), description, id))
